    ...
    return(
      <article key={DP.key}  className='flex flex-col gap-2 shadow-xl border-2 border-gray-300 rounded-xl ' id={DP.id}>
        ...
        <div className='w-[100%]  p-2 flex justify-center gap-2'>
          <button onClick={()=>{LinkDeploy(DP.LinkDeploy)}} className=' h-10 w-[85%] border-1 rounded-xl bg-green-500 text-white shadow-2xs cursor-pointer transition-all duration-200 hover:opacity-75'>See project</button>
          <button onClick={()=>{LinkGithud(DP.LinkGithud)}} className='flex justify-center items-center h-10 w-[15%] border-1 rounded-xl bg-gray-900 text-white shadow-2xs cursor-pointer text-2xl transition-all duration-200 hover:opacity-75'><IoLogoGithub/></button>
        </div>
    </article>
    )
  })
